...
/*
The air velocity is calculated based on prevailing wing and the vel of the body.
prevailing wind is a vector contain both the direction and magnitude of the wing.
*/
void Aero::updateForceFromTensor(RigidBody* body, real duration, const Matrix3& tensor)
{
	...
	//calculate the velocity in body coord
	Vector3 bodyVel = body->getTransform().transformInverseDirection(velocity);

	//Calculate the force in body coord
