...
namespace Engine
{
	namespace Math
	{
		...
		public:
			...
			explicit Mat4(Scalar s)
				: m_col{ s, 0, 0, 0,
				0, s, 0, 0,
				0, 0, s, 0,
				0, 0, 0, s } { }

			explicit Mat4(const Scalar* colMatArray)
			{
				memcpy(m_col->data(), colMatArray, sizeof(Scalar) * 16);
			}

			explicit Mat4(const Vector4<Scalar>& col1,
				const Vector4<Scalar>& col2,
				const Vector4<Scalar>& col3,
				const Vector4<Scalar>& col4)
				: m_col{ col1, col2, col3, col4 } { }

			Mat4(const Mat4<Scalar>& m)
				: m_col{ m.m_col[0], m.m_col[1], m.m_col[2], m.m_col[3] } { }

			Mat4<Scalar>& operator=(const Mat4<Scalar>& m)
			{
				m_col[0] = m.m_col[0];
				m_col[1] = m.m_col[1];
				m_col[2] = m.m_col[2];
			...
			Mat4<Scalar> operator-()
			{
				return Mat4(-m_col[0], -m_col[1], -m_col[2], -m_col[3]);
			}
			...
			Mat4<Scalar>& operator*=(const Mat4<Scalar>& t)
			{
			...
			Vector4<Scalar>& operator[](unsigned i)
			{
				CG_DEBUG_ASSERT(i >= 0 && i <= 3);
				return m_col[i];
			}

			Vector4<Scalar> operator[](unsigned i) const
			{
				CG_DEBUG_ASSERT(i >= 0 && i <= 3);
				return m_col[i];
			}
			...
			Vector4<Scalar> m_col[4];
		};

		template<typename Scalar>
		inline Mat4<Scalar> operator*(const Mat4<Scalar>& m, Scalar s)
		{
			return Mat4<Scalar>(m.m_col[0] * s, m.m_col[1] * s, m.m_col[2] * s, m.m_col[3] * s);
		}

		template<typename Scalar>
		inline Mat4<Scalar> operator*(Scalar s, const Mat4<Scalar>& m)
		{
			return Mat4<Scalar>(s * m.m_col[0], s * m.m_col[1], s * m.m_col[2], s * m.m_col[3]);
		}

		template<typename Scalar>
		inline Mat4<Scalar> operator+(const Mat4<Scalar>& a, const Mat4<Scalar>& b)
		{
			return Mat4<Scalar>(a.m_col[0] + b.m_col[0], a.m_col[1] + b.m_col[1],
		...
		inline Mat4<Scalar> operator/(const Mat4<Scalar>& a, const Mat4<Scalar>& b)
		{
			return Mat3<Scalar>(a.m_col[0] / b.m_col[0], a.m_col[1] / b.m_col[1],
				a.m_col[2] / b.m_col[2], a.m_col[3] / b.m_col[3]);
		}

		template<typename Scalar>
		inline bool operator==(const Mat4<Scalar>& a, const Mat4<Scalar>& b)
		{
		...
		template<typename Scalar>
		inline Mat4<Scalar> Transpose(const Mat4<Scalar>& m)
		{
			Mat4<Scalar> r;
			r.m_col[0][0] = m[0][0];
			r.m_col[0][1] = m[1][0];
		...
		template<typename Scalar>
		inline Scalar Determinant(const Mat4<Scalar>& m)
		{
		...
		template<typename Scalar>
		inline Mat4<Scalar> Inverse(const Mat4<Scalar>& m)
		{
			// https://github.com/willnode/N-Matrix-Programmer
			// I need optimized one. That's why i refer to the link above
			// My Matrix is Column Major, So I inverted the order of accessing the element.
